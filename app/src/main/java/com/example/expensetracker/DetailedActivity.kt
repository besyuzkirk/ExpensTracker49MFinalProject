...
import kotlinx.android.synthetic.main.activity_detailed.closeBtn
...
class DetailedActivity : AppCompatActivity() {
    ...
    private lateinit var transaction : Expense
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        transaction = intent.getSerializableExtra("transaction") as Expense
        ...
        updateBtn.setOnClickListener {
            ...
            if(label.isEmpty())
                ...
                val transaction  = Expense(transaction.id, label, amount, description)
                update(transaction)
            }
        }
    ...
    private fun update(transaction: Expense){
        val db = Room.databaseBuilder(this,
            AppDatabase::class.java,
            "transactions").build()

        GlobalScope.launch {
            db.expenseDao().update(transaction)
            finish()
        }
    }
